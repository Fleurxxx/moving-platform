 ...
 * @LastEditTime: 2024-04-11 20:31:24
    ...
    <div class="container">
      <div class="left">
        ...
        <el-card class="l_card">
          <Input @send-data="handleDataFromChild"/>
        </el-card>
      ...
      <div class="right">
        <Affix :offset-top="50">
          ...
          <div class="r_search">
            <u-search class="search" :config="config" @submit="submit"></u-search>
          </div>
          <div class="r_hot">
            <el-card>
              <h3>资讯推荐</h3>
...
import { Share, Comment } from '@element-plus/icons-vue'
...
const discussList = ref([
  {
    ...
    name:'汤姆·杰森',
    content:`大会很深刻
    ![图片描述](https://s2.loli.net/2024/04/03/57w8v3bRUgBypkl.jpg)
    `,
  },
  {
    id: '234',
    ...
    name:'青风扶林',
    content:`太有到理了！
    ![图片描述](https://obs-xhlj.obs.cn-east-3.myhuaweicloud.com/2023/5/ecf262789a864dc19d2a9cb57739edec.jpg)
    `,
  },
  {
    id: '234',
    ...
    name:'酸酸的',
    content:`好牛！
    ![图片描述](https://obs-xhlj.obs.cn-east-3.myhuaweicloud.com/2023/5/526899a3e3f7459da9c6a3120d1a9b84.jpg)
    `,
  }
])
//资讯推荐
...
const config = ref<SearchConfig>({
  search: '', //可选输入框默认值
  keywords: ['西湖论剑', '数字安全', '安全特训营', '凡人修仙传', '一念永恒'], // 搜索框关键字滚动
  hotSearchList: [
    '西湖论剑',
    '数字安全',
    '安全特训营',
...
function handleDataFromChild(data:any) {
  ...
  console.log(newData);
  // // 向数组开头插入新的数据(浅拷贝)
  // discussList.value.unshift(newData);
  // 对新数据进行深拷贝
  const clonedData = cloneDeep(newData);
  // 在原数组开头插入新数据副本
  discussList.value.unshift(clonedData);
}
//<div id="contentDisplay"></div>
// // 调用函数显示内容
// this.displayHTMLContent(content);
// function displayHTMLContent(content:any) {
//     // 获取要显示内容的容器元素
//     const container = document.getElementById('contentDisplay');
//     // 设置容器元素的innerHTML，即将内容显示在页面上
