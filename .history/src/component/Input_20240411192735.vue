...
export default {
    ...
    methods: {
        ...
        addTopic(){
            ...
            let selection = window.getSelection();
            range.setStart(inputArea.firstChild.nextSibling, 0); // 设置范围的起点为“话题”两个字的起始位置
            range.setEnd(inputArea.firstChild.nextSibling, 2); // 设置范围的终点为“话题”两个字的结束位置
            selection.removeAllRanges();
            selection.addRange(range);
            
            // 可选：如果需要自动显示表情区域，可以添加下面的代码
            this.emojiShow = true;
        },
        //缩略图
