 ...
 * @LastEditTime: 2024-04-03 21:12:54
          ...
          <div class="r_hot">
            <el-card>
              ...
              <div v-for="(item,index) in hot" :key="index">
                <p><span :style="{ color: item.color }">{{ index+1 }}</span> &nbsp;{{ item.Y }}</p>
              </div>
            </el-card>
          </div>
...
import { ref, reactive,defineEmits, toRefs, onBeforeMount, onMounted, watchEffect, computed } from 'vue';
...
function handleDataFromChild(data:any) {
  ...
  // 使用深拷贝进行复制
  const newList = cloneDeep(discussList.value);
