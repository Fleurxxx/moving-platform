...
export default {
  ...
  mounted() {
    ...
    // 检测浏览器是否支持
    this.aliRts.isSupport({ isReceiveVideo: true })
      .then(this.subscribeRts)
      .catch(this.fallback);
  },
  methods: {
    subscribeRts() {
      this.aliRts.subscribe(this.pullStreamUrl)
        .then((remoteStream) => {
          // 播放远程流
          remoteStream.play(this.mediaElement);
        })
        .catch((error) => {
          console.error('Subscribe RTS error:', error);
        });
    },
    fallback() {
      // 降级至其他协议播放
      // 此处为示例，使用HLS播放器播放降级地址
      if (Hls.isSupported()) {
        // 创建HLS播放器实例
        this.hlsPlayer = new Hls();
        this.hlsPlayer.loadSource(this.fallbackUrl);
