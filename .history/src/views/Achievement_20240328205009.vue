<template>
  <div >
    ...
    <div class="center">
      ...
      <el-card class="card" @mouseenter="showContent" @mouseleave="hideContent">
...
const data = reactive({
  productName:'11周年精华特刊',
  productSrc: require('@/assets/image/common/book.png'),
  productIntruction:'',
})

let isContentVisible = ref
function showContent() {
  data.isContentVisible = true;
}
...
.card_content{
  ...
  transition: left 0.3s ease, opacity 0.3s ease; /* 过渡效果 */
  opacity: 0; 
  color: #fff;
  // display: flex;
  // justify-content: center;
  // align-items: center;
  // transition: left 0.3s ease; /* 左侧滑入动画 */
}
.card_content.show {
  left: 0; /* 左侧滑入 */
  opacity: 1; /* 显示内容 */
